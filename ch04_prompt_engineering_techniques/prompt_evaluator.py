...
class PromptEvaluator:
    ...
    def __init__(self, chat_helper, max_concurrent_tasks=3):
    ...
    def generate_unique_ideas(self, task_description, prompt_inputs_spec, num_cases):
        ...
        self.chat_helper.add_user_message(messages, rendered_prompt)
        self.chat_helper.add_assistant_message(messages, "```json")
        
        text = self.chat_helper.chat(
            messages,
            stop_sequences=["```"],
    ...
    def grade_output(self, test_case, output, extra_criteria):
        ...
        * The full 1-10 scale should be utilized - don't hesitate to give low scores when warranted
        ...
        eval_text = self.chat_helper.chat(
            messages,
            stop_sequences=["```"],
            temperature=0.0,
        )
